...
// Function to send reminder email
const sendReminderEmail = async (clientName: string, email: string, courtDate: string) => {
  const subject = 'Court Date Reminder';
  const html = `
    <h2>Court Date Reminder</h2>
    <p>Dear ${clientName},</p>
    <p>This is a reminder that your court date for your 'Traffic Accident' case is scheduled for ${new Date(courtDate).toDateString()}.</p>
    <p>Please ensure you are prepared.</p>
    <p>Best regards,<br>Your Legal Team</p>
  `;
    ...
    try {
      ...
      for (const caseItem of cases) {
        ...
        if (user && user.email && caseItem.coatDate && caseItem.isMailSent === false) {
          await sendReminderEmail(caseItem.clientName, user.email, caseItem.coatDate);
